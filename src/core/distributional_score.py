...
class DistributionalScorer:
    ...
    def _compute_convergence(self, per_group: Dict[str, float], all_nets: List[float]) -> float:
        ...
        total_score = abs(per_group.get("HTF", 0)) + abs(per_group.get("MID", 0)) + abs(per_group.get("LTF", 0))
        positive = 0
        if per_group.get("HTF", 0) > 0:
            positive += abs(per_group["HTF"])
        if per_group.get("MID", 0) > 0:
            positive += abs(per_group["MID"])
        if per_group.get("LTF", 0) > 0:
            positive += abs(per_group["LTF"])
    ...
    def _compute_conviction(self, abs_net: float, std: float,
        ...
        conv = 0.3 * score_magnitude + 0.3 * stability + 0.3 * convergence + 0.1 * regime.confidence
