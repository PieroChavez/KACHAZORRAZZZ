...
class StrategyEngine:
    ...
    def evaluate(self, timeframes: Dict[str, pd.DataFrame],
        ...
        if best.direction in ("BUY", "SELL") and ctx.htf_structure.trend in (TrendDirection.BULLISH, TrendDirection.BEARISH):
            htf_dir = "BUY" if ctx.htf_structure.trend == TrendDirection.BULLISH else "SELL"
            if best.direction != htf_dir:
                best.score -= 5.0
                best.score_breakdown["htf_misalignment_penalty"] = -5.0
                best.notes.append(f"HTF {htf_dir} en contra de {best.direction}, -5pts")

        total_tfs = sum(1 for v in timeframes.values() if v is not None and len(v) > 20)
        if total_tfs >= 3:
            from src.utils.helpers import find_swing_points
            cascade_dir = None
            for df in self._all_timeframes_sorted(timeframes):
                ...
                if cascade_dir is None:
                    cascade_dir = tf_dir
                elif cascade_dir != tf_dir:
                    cascade_dir = "HOLD"
                    break
            if best.direction in ("BUY", "SELL") and cascade_dir and cascade_dir != "HOLD" and best.direction != cascade_dir:
                best.score *= 0.7
                best.notes.append(f"Multi-TF {cascade_dir} vs {best.direction}, -30%")
    ...
    def evaluate_adaptive(self, timeframes: Dict[str, pd.DataFrame],
        ...
        if direction == "HOLD" or conviction < 0.3:
            ...
            return TradingSignal(
                ...
                notes=[f"Convicción {conviction:.0%} < 30%"] + dist.notes + best.notes,
        ...
        if total_tfs >= 3:
            ...
            for df in self._all_timeframes_sorted(timeframes):
                ...
                if cascade_dir is None:
                    ...
                    cascade_dir = "HOLD"
                    break
            if direction in ("BUY", "SELL") and cascade_dir and cascade_dir != "HOLD" and direction != cascade_dir:
                conviction *= 0.7
                dist.conviction = conviction
                dist.notes.append(f"Multi-TF {cascade_dir} vs {direction}, convicción -30%")
    ...
    def _evaluate_direction(self, direction: str, ctx: MarketContext,
        ...
        if fvg_match:
            score += self.weights.fvg_detected * fvg_match.confidence
            breakdown["fvg_detected"] = self.weights.fvg_detected * fvg_match.confidence
        ...
        if ob_match:
            score += self.weights.order_block_valid * ob_match.confidence
            breakdown["order_block_valid"] = self.weights.order_block_valid * ob_match.confidence
        ...
        if breaker_match:
            score += self.weights.breaker_retest * breaker_match.confidence
            breakdown["breaker_retest"] = self.weights.breaker_retest * breaker_match.confidence
        ...
        if cycle_match:
            score += (self.weights.slip_memory_present + self.weights.equidad_sweep_confirmed + self.weights.eslabon_breakout)
            breakdown["cycle_full"] = (self.weights.slip_memory_present + self.weights.equidad_sweep_confirmed + self.weights.eslabon_breakout)
        ...
        if void_scalp_match:
            score += self.weights.void_scalp_confirmed * void_scalp_match.confidence
            breakdown["void_scalp"] = self.weights.void_scalp_confirmed * void_scalp_match.confidence
        ...
        if bos_zone_match:
            score += self.weights.bos_zone_retest * bos_zone_match.confidence
            breakdown["bos_zone_retest"] = self.weights.bos_zone_retest * bos_zone_match.confidence
        ...
        if pe_match:
            score += self.weights.price_establishment_bonus * pe_match.confidence
            breakdown["price_est"] = self.weights.price_establishment_bonus * pe_match.confidence
        ...
        if self._has_ltf_sweep_confirmation(ctx, ltf_df):
            score += self.weights.ltf_sweep_confirmation
            breakdown["ltf_sweep_confirmation"] = self.weights.ltf_sweep_confirmation

        if self._has_multiframe_alignment(ctx):
            score += self.weights.multiframe_alignment
            breakdown["multiframe_alignment"] = self.weights.multiframe_alignment

        triple = self._check_triple_confluence(ltf_patterns, direction)
        if triple:
            score += self.weights.triple_confluence
            breakdown["triple_confluence"] = self.weights.triple_confluence
        ...
        # Interval Point (Punto de Intervalo, Clase 13, 15)
        ...
        if ip_match:
            score += self.weights.interval_point_bonus * ip_match.confidence
            breakdown["interval_point"] = self.weights.interval_point_bonus * ip_match.confidence
            notes.append(f"Punto de Intervalo: mechas {ip_match.extra.get('wick_ratio', 0):.1f}× cuerpo")
            if primary is None:
                primary = ip_match

        # Price Interaction (Interacción de Precio, Clase 12, 16, 23, 24)
        ...
        if pi_match:
            score += self.weights.price_interaction_bonus * pi_match.confidence
            breakdown["price_interaction"] = self.weights.price_interaction_bonus * pi_match.confidence
            notes.append(f"Interacción precio: nivel {pi_match.extra.get('interacted_level', 0):.2f}")
            if primary is None:
                primary = pi_match

        # Harmonic Cycle (Ciclo Armónico, Clase 11, 18)
        ...
        if hc_match:
            score += self.weights.harmonic_cycle_aligned * hc_match.confidence
            breakdown["harmonic_cycle"] = self.weights.harmonic_cycle_aligned * hc_match.confidence
            notes.append(f"Ciclo Armónico: 50% en {hc_match.mid:.2f}")

        # Pressure Zone (Zona POD, Clase 25)
        ...
        if pz_match:
            score += self.weights.pressure_zone_bonus * pz_match.confidence
            breakdown["pressure_zone"] = self.weights.pressure_zone_bonus * pz_match.confidence
    ...
    def _compute_pattern_confluence(self, ltf_patterns: dict, direction: str, ltf_df: pd.DataFrame) -> float:
        ...
        multiplier = 1.0 - (density * 0.25) - (overlap_ratio * 0.25)
