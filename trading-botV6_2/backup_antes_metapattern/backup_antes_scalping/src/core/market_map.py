...
class MarketMap:
    ...
    def evaluate(self, dfs: Dict[str, pd.DataFrame],
        ...
        if not phase.allows_entry and not breakout_signals:
            if conviction >= self.MIN_CONVICTION_BYPASS:
                result_notes.append(
                    f"Fase {phase.phase.value} sin breakout, pero convicción {conviction:.0%} ≥ "
                    f"{self.MIN_CONVICTION_BYPASS:.0%} → bypass"
                )
            else:
        ...
        if not route.is_valid:
            result_notes.append(f"Ruta inválida: {route.notes[0] if route.notes else ''}")
            if breakout_signals:
                result_notes.append("Breakout señal detectada pero inactiva, esperando retest")
            return MarketMapResult(
                decision="WAIT" if breakout_signals else "NO_TRADE",
                direction=direction, confidence=phase.confidence,
        ...
        candle_results = {}
        if df_h1 is not None and self._candle_confirmer is not None:
            try:
                candle_results = self._candle_confirmer.check(df_h1, df_h4, direction, liquidity_map)
            except Exception:
                logger.exception("CandleConfirmer failed")
                candle_results = {}
        confirmed_1h = candle_results.get("1h", CandleConfirmerResult(
            status=ConfirmerStatus.NOT_READY, direction=direction, timeframe="N/A",
        )).is_confirmed
        confirmed_4h = candle_results.get("4h", CandleConfirmerResult(
            status=ConfirmerStatus.NOT_READY, direction=direction, timeframe="N/A",
        )).is_confirmed

        for tf_label, cr in candle_results.items():
            result_notes.append(f"Confirmación {tf_label}: {cr.status.value}")
        ...
        sl_price = route.sl_zone_price if route.sl_zone_price else (
            (entry.suggested_entry or 0) - atr_val * 0.8 if direction == "BUY"
            else (entry.suggested_entry or 0) + atr_val * 0.8
        )
        ...
        if binary.passed:
            ...
            result_notes.append(f"F3 BinaryRisk FAIL: {binary.failed_checks}")
            return MarketMapResult(
                decision="NO_TRADE", direction=direction, confidence=0.0,
                market_map=liquidity_map, phase=phase,
                route=route, entry_confirmation=entry,
                candle_confirmation=candle_results,
                tp_tiers=tp_tiers, active_tp=active_tp,
                suggested_entry=entry.suggested_entry, stop_loss=sl_price,
                score_bonus=md_score_bonus, final_score=0.0,
                md_detections=md_detections,
                notes=result_notes,
            )

        final_confidence = phase.confidence * 0.2 + route.confidence * 0.3 + entry.confidence * 0.3
        if confirmed_1h:
