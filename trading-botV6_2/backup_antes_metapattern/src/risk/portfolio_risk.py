...
class PortfolioRiskManager:
    ...
    def get_net_directional_penalty(
        ...
        hypothetical_volume = self._estimate_trade_notional(symbol)
        new_net = current_net + side * hypothetical_volume
        ...
        gross = exp.gross_notional + hypothetical_volume * self._get_contract_size(symbol) * self._estimate_entry(symbol)
        if gross > max_gross:
    ...
    def _estimate_trade_notional(self, symbol: str) -> float:
        notional = self.balance * 0.02 * self._get_contract_size(symbol)
        entry = self._estimate_entry(symbol)
