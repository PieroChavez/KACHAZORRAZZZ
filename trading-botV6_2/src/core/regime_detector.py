...
Supports ML-based classification via Random Forest (Mejora 2).
"""
import logging
...
class RegimeContext:
    ...
    _current_timing: str = "ALL"
    _current_session: str = "ALL"

    def set_context(self, timing: str = "ALL", session: str = "ALL"):
        self._current_timing = timing
        self._current_session = session

    def get_multiplier(self, pattern_type: str) -> float:
        base = self.pattern_multipliers.get(pattern_type, 1.0)
        if self._current_timing != "ALL":
            timing_key = f"{pattern_type}|{self._current_timing}"
            base *= self.pattern_multipliers.get(timing_key, 1.0)
        if self._current_session != "ALL":
            session_key = f"{pattern_type}|{self._current_session}"
            base *= self.pattern_multipliers.get(session_key, 1.0)
        return round(base, 2)


ML_LABEL_MAP = {
    0: RegimeType.STRONG_TREND_BULLISH,
    1: RegimeType.STRONG_TREND_BEARISH,
    2: RegimeType.RANGING,
    3: RegimeType.HIGH_VOLATILITY,
    4: RegimeType.LOW_VOLATILITY,
    5: RegimeType.TRANSITION,
}


class RegimeDetector:
    def __init__(self, atr_period: int = 14, adx_period: int = 14,
                 symbol: str = "", model_dir: Optional[Path] = None):
        self.atr_period = atr_period
        self.adx_period = adx_period
        self._prev_regime: Optional[RegimeType] = None
        self._regime_stability: int = 0
        self._symbol = symbol
        self._ml_model_data: Optional[Dict] = None
        self._ml_available = False
        self._load_ml_model(model_dir)

    def _load_ml_model(self, model_dir: Optional[Path] = None) -> bool:
        if not self._symbol or not model_dir:
            return False
        path = model_dir / f"regime_rf_{self._symbol}.pkl"
        if not path.exists():
            logger.info(f"[{self._symbol}] No ML regime model found at {path}, usando reglas")
            return False
        try:
            import pickle
            with open(path, "rb") as f:
                self._ml_model_data = pickle.load(f)
            self._ml_available = True
            acc = self._ml_model_data.get("test_accuracy", 0)
            feat = dict(list(self._ml_model_data.get("feature_importance", {}).items())[:5])
            logger.info(f"[{self._symbol}] ML regime model loaded: test_acc={acc:.1%}, "
                        f"top_features={feat}")
            return True
        except Exception as e:
            logger.warning(f"[{self._symbol}] Failed to load ML regime model: {e}")
            self._ml_available = False
            return False

    def _ml_predict(self, ltf_df: pd.DataFrame) -> Optional[Tuple[RegimeType, float]]:
        if not self._ml_available or self._ml_model_data is None:
            return None
        try:
            from src.core.regime_trainer import compute_features, RegimeTrainer
            features = compute_features(ltf_df)
            if len(features) == 0:
                return None
            last_row = features.iloc[[-1]]
            preds, confs, probs = RegimeTrainer.ml_predict(last_row, self._ml_model_data)
            label_id = int(preds[0])
            confidence = float(confs[0])
            regime = ML_LABEL_MAP.get(label_id, RegimeType.TRANSITION)
            return regime, confidence
        except Exception as e:
            logger.warning(f"[{self._symbol}] ML predict error: {e}, usando reglas")
            return None
    ...
    def detect(self, htf_df: pd.DataFrame, ltf_df: pd.DataFrame) -> RegimeContext:
        ...
        ml_result = self._ml_predict(ltf_df)
        if ml_result is not None:
            regime, ml_confidence = ml_result
            ml_note = f"ML-RF"
