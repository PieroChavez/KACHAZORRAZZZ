...
from src.learning.bayesian_ensemble import BayesianEnsemble
...
from src.predictor import MicroPredictor, PredictorConfig, MicroPrediction
...
def build_symbol_profile(cfg: dict) -> SymbolProfile:
    return SymbolProfile(
        ...
        base_volume=cfg.get("base_volume", 0.0),
        isolated_risk=cfg.get("isolated_risk", False),
...
def build_scoring_config(cfg: dict) -> ScoringConfig:
    return ScoringConfig(
        ...
        cycle_full=cfg.get("cycle_full", 15.0),
...
class TradingBot:
    def __init__(self, config_dir: Path):
        ...
        self.bayesian_ensemble = BayesianEnsemble(
            db_path=project_root / "data" / "bayesian_ensemble.db"
        )
        ...
        self._watchdog_timeout = 120  # segundos sin heartbeat → restart
        ...
        self.micro_predictor = MicroPredictor(PredictorConfig(
            model_path=project_root / "data" / "models" / "micro_predictor.pkl",
        ))
        self._last_prediction: Dict[str, MicroPrediction] = {}
    ...
    async def _restore_market_hours(self):
        try:
            ...
            if saved:
                ...
                filtered = {k: v for k, v in saved.items() if k in active_set}
                if filtered:
                    self.hours_controller.restore_states(filtered)
                stale = len(saved) - len(filtered)
                if stale > 0:
                    logger.info(f"[MarketHours] Restored state for {len(filtered)} symbols ({stale} stale entries ignored)")
                    await self.state_persistence.clean_market_hours(active_set)
                else:
                    logger.info(f"[MarketHours] Restored state for {len(filtered)} symbols")
    ...
    def start(self, max_duration: int = 0):
        ...
        self._last_loop_time = time.time()
        self._loop_iteration = 0
        watchdog = threading.Thread(target=self._watchdog_worker, daemon=True)
        watchdog.start()

        state_save_interval = max(1, self.config["strategy"].get("state_save_interval_seconds", 120))
        last_state_save = time.time()

        while self.running:
            self._loop_iteration += 1
            time.sleep(self.config["strategy"].get("loop_sleep_seconds", 5))
            self._last_loop_time = time.time()
            ...
            if self._loop_iteration % 60 == 0:
                uptime = time.time() - self.start_time
                logger.info(f"HEARTBEAT: iter #{self._loop_iteration}, uptime={uptime:.0f}s")

        try:
            self.stop()
        except Exception as e:
            logger.error(f"Error during final stop: {e}")
    ...
    def _watchdog_worker(self):
        while getattr(self, 'running', False):
            ...
            if not getattr(self, 'running', False):
                break
            elapsed = time.time() - self._last_loop_time
            if elapsed > self._watchdog_timeout:
                logger.error(f"WATCHDOG: Loop frozen {elapsed:.0f}s (iter #{self._loop_iteration}). Forcing restart.")
                try:
                ...
                import os as _os
                _os._exit(1)
            logger.info(f"WATCHDOG: alive (iter #{self._loop_iteration}, {elapsed:.0f}s since last iter)")
    ...
    def _calc_volume(self, signal, profile, ltf_df, pip_value, symbol, conviction=None) -> float:
        ...
        if getattr(profile, 'base_volume', 0.0) > 0:
            volume = max(0.01, round(profile.base_volume * vol_mult, 2))
            logger.info(f"[{symbol}] Base volume fijo: {volume} lots (×{vol_mult:.2f})")
            return volume
    ...
    def _record_trade(self, symbol: str, position: dict, profit: float,
        ...
        try:
            ...
            if (sl_price is None or sl_price == 0) and fresh_signal:
                sl_price = fresh_signal.stop_loss
            tp_price = position.get("tp")
            if (tp_price is None or tp_price == 0) and fresh_signal:
                tp_price = fresh_signal.take_profit

            timing = "UNKNOWN"
            try:
                dfs = self.fetcher.get_dataframes(symbol, count=30)
                ltf_key = min(dfs.keys(), key=lambda k: self.fetcher.timeframes_available.index(k) if k in self.fetcher.timeframes_available else 99) if dfs else None
                ltf_df = dfs.get(ltf_key) if ltf_key else None
                if ltf_df is not None and not ltf_df.empty:
                    from src.utils.helpers import atr as _atr
                    atr_series = _atr(ltf_df, 14)
                    atr_val = atr_series.iloc[-1] if not atr_series.empty else 0.0
                    timing = MetaLearner.classify_timing(
                        position.get("price_open", 0), direction, ltf_df, atr_val
                    )
            except Exception:
                pass
            ...
            # ── Bayesian Ensemble: post-trade update ──
            try:
                last_mm = getattr(self, '_last_market_map_conf', {}).get(symbol, 0.0)
                last_of = getattr(self, '_last_of_conf', {}).get(symbol, 0.0)
                last_micro = getattr(self, '_last_micro_conf', {}).get(symbol, 0.0)
                last_align = getattr(self, '_last_regime_alignment', {}).get(symbol, 0.0)
                pip_size_val = pip_size(symbol)
                self.bayesian_ensemble.update_post_trade(
                    symbol=symbol, direction=direction, regime=regime_val,
                    profit=profit, exit_reason=exit_reason,
                    market_map_conf=last_mm, of_confidence=last_of,
                    micro_conf=last_micro, regime_alignment=last_align,
                    entry_price=record.entry_price,
                    stop_loss=sl_price or 0,
                    take_profit=tp_price or 0,
                    exit_price=record.exit_price,
                    pattern_type=record.primary_pattern,
                    pip_size_val=pip_size_val,
                )
            except Exception as e_be:
                logger.debug(f"[Bayesian] Post-trade error: {e_be}")
